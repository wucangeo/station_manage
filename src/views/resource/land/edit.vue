...
export default {
  ...
  methods: {
    ...
    async update() {
      ...
      formData.build_year = formData.build_year || 0 // '建立日期',
      formData.area = formData.area || 0 // '样地面积',
      formData.longitude = formData.longitude || 0 // '经度',
      formData.latitude = formData.latitude || 0 // '纬度',
      formData.altitude = formData.altitude || 0 // '高程',
      formData.is_public = formData.is_public || 0 //'是否对外服务',

      formData.code = formData.code || '' // '样地代码',
      formData.name = formData.name || '' // '样地名称',
      formData.type = formData.type || '' // '样地类型',
      formData.use_year = formData.use_year || '' // '设计使用年限',
      formData.sharp = formData.sharp || '' // '样地形状',
      formData.ready_to = formData.ready_to || '' // '可进行工作',
      formData.desc = formData.desc || '' // '样地代表性描述',
      formData.property = formData.property || '' // '样地性质',
      formData.location = formData.location || '' // '所在地点',
      formData.soil_type = formData.soil_type || '' // '土壤类型',
      formData.topography = formData.topography || '' // '地形地貌',
      formData.vegetation_type = formData.vegetation_type || '' // '植被类型',
      formData.plant_property = formData.plant_property || '' // '植物群落特征',
      formData.manage = formData.manage || '' // '建立后管理措施',
      formData.remark = formData.remark || '' // '备注',

      let response = await this.apis.res_land.update(formData, this.data_id)
      let result = response.data
      if (result.code === 0) {
        this.$Message.error({
          content: result.msg,
